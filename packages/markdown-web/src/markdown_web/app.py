...
from markdown_web.math import MathFormulaError, decode_formula, render_formula_png
...
def math_png(formula_token: str) -> Response:
    """Serve an extracted LaTex formula as an immutable PNG."""
    try:
        image = render_formula_png(decode_formula(formula_token))
    except MathFormulaError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    return Response(image, media_type="image/png", headers={"Cache-Control": "public, max-age=31536000, immutable"})
