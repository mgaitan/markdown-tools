...
def test_extract_main_content_keeps_html_author_over_structured_publisher(*, mocker: MockerFixture) -> None:
    html = """
    <meta name="author" content="Reporter Name">
    <script type="application/ld+json">
      {"@type": "NewsArticle", "articleBody": "Article body.", "publisher": {"name": "Example News"}}
    </script>
    <article><p>Article body.</p></article>
    """
    mocker.patch.object(extractor_module, "Document", return_value=_document(mocker=mocker))
    ...
    metadata, _body = split_front_matter(markdown)
    assert metadata["author"] == "Reporter Name"
